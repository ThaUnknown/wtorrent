...
const Navbar = element => {
  return (
    <nav className='navbar p-0 bg-dark-dm bg-white-lm border-bottom-0'>
      ...
      <div className='window-controls min' onClick={() => window.minimize()}>
        <svg width='1rem' height='1rem' viewBox='0 0 12 12'>
          <rect fill='currentColor' width='10' height='1' x='1' y='6' />
        </svg>
      </div>
      <div className='window-controls max' onClick={() => window.isMaximized() ? window.unmaximize() : window.maximize()}>
        <svg width='1rem' height='1rem' viewBox='0 0 12 12'><rect width='9' height='9' x='1.5' y='1.5' fill='none' stroke='currentColor' />
        </svg>
      </div>
      <div className='window-controls close' onClick={() => window.close()}>
        <svg width='1rem' height='1rem' viewBox='0 0 12 12'>
